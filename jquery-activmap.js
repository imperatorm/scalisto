...
(function($){
    ...
    $.activmap = {
        ...
        defaults: {
            ...
            zoom: 6,              //default zoom level between 0 and 21
            maxZoom: 12,
    ...
    $.fn.extend({
        ...
        activmap : function(settings){
            ...
            _order = function(){
                $.each(s.places, function(i, place){ //console.log(place);
                    place.dist = _get_distance(place.marker.position, latlng);
                });
                // s.places.sort(_sort_by_dist); //NO! Leave the list alone
            ...
            _init = function(){
                ...
                $.each(s.places, function(i, place){
					...
					if(place.custom_content != undefined && place.custom_content != '') mycontent += place.custom_content+'<br />';
                ...
                $('#activmap-reset').on('click', function(){
                    $('input[name="marker_type[]"], select[name="marker_type[]"]').prop('checked',true);
                    $('input[name="marker_type[]"], select[name="marker_type[]"]').each(function(){
                        _update_places_tag($(this));
                    });
                    latlng = init_latlng;
